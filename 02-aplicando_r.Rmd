...
```
Como se puede observar, la línea recta de color verde corta el eje Y en el valor del *intercept* (3.8581), cuyo valor está almacenado en la variable `reg`. Abajo he escrito un ejemplo maas con las variables `emociones_negativas` y `promedio`. Basicamente, sigue la misma lógica de las variables escritas arriba, solo modificando el nombre de la variable `reg_1` para este caso y modifiqué el color de la línea a color rojo. ¿Qué tipo de relación hay entre estas dos variables?

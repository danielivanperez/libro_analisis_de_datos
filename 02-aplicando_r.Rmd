...
tail(dataset)
```
Finalmente, terminar diciendo que si queremos ver un número determinado de observaciones de la base de datos usando la función `head()` o `tail()`, podemos escribir el número de filas después de `dataset` en nuestro caso, separado por una coma. A continuación, muestro un ejemplo para ver las primeras 10 filas de la base de datos: 

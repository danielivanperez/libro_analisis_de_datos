...
Además de poder seleccionar columnas o variables en una base de datos, podemos seleccionar a conveniencia las filas o participantes (para este ejemplo) que queramos. Imaginemos que queremos elegir solo las mayores de 20 años de nuestra base de datos. Para esto vamos a utilizar la función `subset()` que nos permite seleccionar un subconjunto de filas de una base de datos determinada. Procederemos a ejecutar un pequeño ejemplo abajo. 

```{r}
subset(dataset, edad > 20) 
```
...
```{r}
subset(dataset, edad > 20 | edad == 18)
```
